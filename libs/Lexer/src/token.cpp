...
lexer::Token::Token() : _id(0), _defineId(defineTokenId) {}

lexer::Token::Token(const std::wstring& text) : _text(text), _defineId(defineTokenId) {
    _updateId();
}
...
constexpr std::uint64_t lexer::defineTokenId(const wchar_t* token) {
    std::uint64_t hash = 0xcbf29ce484222325;  // FNV0 hash = 0
    for (std::size_t i = 0; token[i] != L'\0'; ++i) {
        hash ^= static_cast<std::uint64_t>(token[i]);
